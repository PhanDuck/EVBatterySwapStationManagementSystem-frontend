...
export default function DriverSubscriptionManagement() {
  ...
  const handleCalculateRenewal = async (renewalPackageId) => {
    ...
    try {
      ...
      message.error(error.response?.data?.message || "Không thể tính toán chi phí gia hạn!");
    } finally {
  ...
  const handleConfirmRenewal = async () => {
    ...
    try {
      ...
      message.error(error.response?.data?.message || "Không thể bắt đầu gia hạn!");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const columns = [
    ...
    ...(role === "DRIVER" ? [{ title: "Thao tác", key: "actions", fixed: "right", render: (_, record) => (
        <Space>
          ...
          <Button icon={<ReloadOutlined />} type="primary" onClick={() => openRenewalModal(record)} disabled={record.status !== "ACTIVE"}>Gia hạn</Button>
        </Space>
    )}] : []),
  ];
        ...
        {renewalCalculation && (
          <Card style={{ marginTop: 16 }} bordered={false}>
            ...
            <List itemLayout="horizontal">
              ...
              <List.Item>
                <List.Item.Meta title="Phí phạt (nếu có)" description={formatCurrency(renewalCalculation.penaltyFee)} />
              </List.Item>
              <List.Item>
                  <List.Item.Meta title="Tổng tiền thanh toán" description={<Text strong color="green">{formatCurrency(renewalCalculation.finalPrice)}</Text>} />
              </List.Item>
            </List>
            <Divider />
            <Alert
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Modal title="Gia hạn gói cước" visible={isRenewalModalVisible} onCancel={() => setIsRenewalModalVisible(false)} footer={[
        <Button key="back" onClick={() => setIsRenewalModalVisible(false)}>Hủy</Button>,
        <Button key="submit" type="primary" loading={submitting} onClick={handleConfirmRenewal} disabled={!targetPackageId || isCalculatingRenewal}>
          Xác nhận & Thanh toán
        </Button>,
      ]} width={700}>
        <p><Text strong>Gói hiện tại:</Text> {packageName(selectedSubscription?.packageId)}</p>
        <Form layout="vertical">
          <Form.Item label="Chọn gói để gia hạn:">
            <Select placeholder="Chọn một gói" onChange={handleCalculateRenewal} value={targetPackageId}>
              {packages.map((p) => (
                <Option key={p.id} value={p.id}>{p.name} - {p.price.toLocaleString()}đ</Option>
              ))}
            </Select>
